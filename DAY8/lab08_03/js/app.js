 ...
 *      예외가 발생했을 때 처리하는 부분
 * }finally{
 *      try,catch 안에 있는 내용이 다 실행되고나서 반드시 실행되는 부분
 * }
 * 
 * try {
 *      예외가 발생할 수 있는 코드
 ...
 *          // ReferenceError 발생 시 처리할 내용을 작성
 *      }else{
 *          // 그 밖의 예외를 처리한다
 *      }
 * }
...
function updateCheck(user){
    // 게임을 사용하는 유저의 프로그램 버전이 3보다 적으면, 빠져나가게 한다.
    if(user.version < 3){
        return; // 함수를 종료하고 빠져 나간다.
    }
}
 ...
 *  - 그 동안 async 함수는 이러는 비동기 작업이 완료될 때까지 일시 중지 상태로 비동기 작업(Promise)의 해결(resolve)을 기다린다.
